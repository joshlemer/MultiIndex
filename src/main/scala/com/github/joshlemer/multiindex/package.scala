...
package object multiindex {
  ...
  implicit class MultiIndexIterableOps[A](iterable: Iterable[A]) {
    val sequence = iterable.toSeq

    def indexBy[B1](f1: A => B1): MultiIndex1[A, B1] = new MultiIndex1Impl(iterable.toMultiSet, f1, Index.fromIterable(sequence)(f1))
